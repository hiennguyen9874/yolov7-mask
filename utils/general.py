...
def non_max_suppression_att(
    ...
    t = time.time()
    output = [torch.zeros((0, 6), device=prediction.device)] * prediction.shape[0]
    output_att = [torch.zeros((0, 980), device=prediction.device)] * prediction.shape[0]
    for xi, x in enumerate(prediction):  # image index, image inference
        # Apply constraints
        # x[((x[..., 2:4] < min_wh) | (x[..., 2:4] > max_wh)).any(1), 4] = 0  # width-height
        x = x[xc[xi]]  # confidence
...
def non_max_suppression_mask_conf(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        print("A")
        print(base[None].shape)
        print(box.shape)

        pooled_bases = pooler([base[None]], [bboxes])

        print("B")
        print(pooled_bases.shape)
        print(a.shape)
