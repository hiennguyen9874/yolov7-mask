    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            if self.attn is not None:
                ...
                attn[i] = (
                    attn[i].view(bs, self.na, self.attn, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
                )
            if self.mask_iou:
            ...
            if not self.training:  # inference
                ...
                y = x[0][i].sigmoid()
                if not torch.onnx.is_in_onnx_export():
                    y[..., 0:2] = (y[..., 0:2] * 3.0 - 1.0 + self.grid[i]) * self.stride[i]  # xy
                    y[..., 2:4] = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i]  # wh
                else:
                    xy, wh, conf = y.split(
                        (2, 2, self.nc + 1), 4
                    )  # y.tensor_split((2, 4, 5), 4)  # torch 1.8.0
                    xy = xy * (3.0 * self.stride[i]) + (
                        self.stride[i] * (self.grid[i] - 1.0)
                    )  # new xy
                    wh = wh**2 * (4 * self.anchor_grid[i].data)  # new wh
                    y = torch.cat((xy, wh, conf), 4)
                z.append(y.view(bs, self.na * ny * nx, self.no))
